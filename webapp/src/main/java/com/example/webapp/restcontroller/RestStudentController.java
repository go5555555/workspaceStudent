...
import com.example.webapp.dto.OneStudentDTO;
...
public class RestStudentController {
	...
	//生徒情報を更新するための画面へ遷移
	@GetMapping("/form/{id}")
	public OneStudentDTO changeStudent(@PathVariable Integer id) {
		StudentView view = studentService.findOneService(id);

		if (view == null) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "生徒が見つかりません");
		}
		OneStudentDTO osd = new OneStudentDTO();
		osd.setId(id);
		osd.setStudentName(view.getStudentName());
		osd.setGrade(view.getGrade());
		osd.setIsNew(false);
		return osd;
	}
	...
	//formで受け取りvalidetedしてerrorがなければぬける
	@PostMapping("/update")
	public ResponseEntity<?> updateStudent(@RequestBody @Validated StudentForm form, BindingResult bindingResult) {
		if (bindingResult.hasErrors()) {
			Map<String, String> errors = new HashMap<>();
			//getFielsErrorsで１つ以上のerrorを取得。
			//bindingResultにerrorフィールドがあるからそれを引数にしてるラムダ式
		...
		if (form.getStudentName().isEmpty() && form.getGrade().equals(0)) {
		...
		List<Student> students = studentService.findAllService();
		StudentDTO sd = new StudentDTO("生徒情報を更新しました", students);
		return ResponseEntity.ok(sd);
	}
	//		//生徒情報削除の確認画面へ遷移
